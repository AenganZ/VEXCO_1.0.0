...
MAPPING_TABLE = {
    ...
    # code_not_present -> vulnerable_code_not_present (component_not_present 아님)
    ...
    # CSAF justification → CycloneDX 매핑 (interlynk 참조)
    "csaf_justification_to_cyclonedx": {
        "component_not_present": "requires_dependency",
        "vulnerable_code_not_present": "code_not_present",
        "vulnerable_code_not_in_execute_path": "code_not_reachable",
        "vulnerable_code_cannot_be_controlled_by_adversary": "requires_environment",
    ...
    # CycloneDX justification → CSAF 매핑
    "cyclonedx_justification_to_csaf": {
        "code_not_present": "vulnerable_code_not_present",
        "code_not_reachable": "vulnerable_code_not_in_execute_path",
        "requires_configuration": "vulnerable_code_cannot_be_controlled_by_adversary",
        "requires_dependency": "inline_mitigations_already_exist",
...
# OpenVEX aliases → CSAF ids 변환용 Alias 시스템 이름 패턴
ALIAS_SYSTEM_PATTERNS = {
...
def get_alias_system_name(alias_id: str) -> Optional[str]:
    """
    접두사 패턴을 기반으로 alias ID의 system_name 결정.
    CVE ID의 경우 None 반환 (ids 필드가 아닌 cve 필드로 가야 함).
    """
    if not alias_id:
        return "External Identification System"
    
    # CVE ID는 건너뛰기 (cve 필드로 가야 함, ids 아님)
    if alias_id.startswith("CVE-"):
        return None
    
    # 알려진 패턴 확인
    for prefix, system_name in ALIAS_SYSTEM_PATTERNS.items():
        if alias_id.startswith(prefix):
            return system_name
    
    # 하이픈 앞의 접두사 추출
    if "-" in alias_id:
        prefix = alias_id.split("-")[0]
...
def csaf_justification_to_cyclonedx(flag: str) -> Optional[str]:
    """CSAF justification 플래그를 CycloneDX justification으로 매핑."""
    return MAPPING_TABLE["csaf_justification_to_cyclonedx"].get(flag)


def cyclonedx_justification_to_csaf(just: str) -> Optional[str]:
    """CycloneDX justification을 CSAF 플래그로 매핑."""
    return MAPPING_TABLE["cyclonedx_justification_to_csaf"].get(just)



# CWE (공통 취약점 열거) 이름 매핑 테이블
# 출처: MITRE CWE 목록 (https://cwe.mitre.org/)
# 버전: 4.16 (2025)
# 전체 항목: 944개 (Weakness만, Category 제외)
CWE_NAMES = {
    # CWE 5-99: J2EE/ASP.NET 설정 오류, 경로 순회, 인젝션
    ...
    "461": "Missing One-Time Initialization",
    ...
    "795": "Only Filtering Special Elements at a Designated Location",
    ...
    "1188": "Initialization with Hard-Coded Network Resource Configuration Data",
...
# =============================================================================
# VEX JUSTIFICATION 매핑
# =============================================================================

# OpenVEX justification 값
...
def is_cwe_category(cwe_id: str) -> bool:
    """CWE ID가 카테고리인지 확인 (Weakness가 아님)."""
    cwe_num = str(cwe_id).replace("CWE-", "").replace("Category-", "").strip()
    return cwe_num in CWE_CATEGORIES


def get_cwe_category_name(cwe_id: str) -> str:
    """ID로부터 CWE 카테고리 이름 가져오기."""
    cwe_num = str(cwe_id).replace("Category-", "").replace("CWE-", "").strip()
    return CWE_CATEGORIES.get(cwe_num)
...
def get_cwe_name(cwe_id: str) -> str:
    ...
    Args:
        cwe_id: CWE 식별자 (예: "79", "CWE-79")
    
    Returns:
        CWE 이름 문자열. 매핑에서 찾지 못하면 "CWE-{id}" 반환.
    """
    cwe_num = str(cwe_id).replace("CWE-", "").strip()
    name = CWE_NAMES.get(cwe_num)
    if name:
        return name
    return f"CWE-{cwe_num}"

# 식별자 패턴을 기반으로 한 컴포넌트 유형 분류
def classify_component_type(identifier: str, name: str = "") -> str:
    """
    PURL, CPE 또는 이름 패턴을 기반으로 컴포넌트 유형 분류.
    CycloneDX 컴포넌트 유형 반환.
    """
...
# ===== TLP (Traffic Light Protocol) 매핑 =====
# 참조:
# - CSAF 2.0: WHITE, GREEN, AMBER, RED
# - CSAF 2.1: CLEAR, GREEN, AMBER, AMBER+STRICT, RED
# - CycloneDX 1.7: CLEAR, GREEN, AMBER, AMBER_AND_STRICT, RED

# CSAF 2.0 → 2.1 변환 (레거시 지원)
TLP_CSAF_2_0_TO_2_1 = {
    "WHITE": "CLEAR",
    "GREEN": "GREEN",
    "AMBER": "AMBER",
...
# CSAF 2.1 → 2.0 변환 (레거시 출력)
TLP_CSAF_2_1_TO_2_0 = {
    "CLEAR": "WHITE",
    "GREEN": "GREEN",
    "AMBER": "AMBER",
    "AMBER+STRICT": "AMBER",  # 손실: STRICT 정보 손실
    "RED": "RED"
}

# CSAF 2.1 ↔ CycloneDX 1.7 (직접 매핑)
...
# OpenVEX는 TLP를 지원하지 않음 - extension_data로 보존 가능

# TLP 기본값
TLP_DEFAULT_LABEL = "CLEAR"
TLP_DEFAULT_URL = "https://www.first.org/tlp/"